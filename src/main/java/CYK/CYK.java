...
public class CYK {
    ...
    public static void main(String[] args){
        ...
        CNF.initialize();
        System.out.println(CYK.isStringInCFG("weather aachen"));
        System.out.println(CYK.isStringInCFG("which lectures are there on monday at 9"));
        System.out.println(CYK.isStringInCFG("where is deepSpace"));
        System.out.println(CYK.isStringInCFG("where is Maastricht"));
    }
    public static boolean isStringInCFG(String query){
        ...
        for(Rule r: RuleEngine.getRules()) {
            if (r.variable.equals("<S>"))
    ...
    public static int[][] ruleYield() {
        ...
        for (Map.Entry<String, List<String>> entry : CNF.getCnf().entrySet()) {
            for (int i = 0; i < entry.getValue().size(); i++) {
                ...
                if (split.length == 2) {
                    Integer index0 = indexMap.get(split[0]);
                    Integer index1 = indexMap.get(split[1]);

                    if (index0 != null && index1 != null) {
                        int[] a = new int[]{
                                indexMap.get(entry.getKey()),
                                index0.intValue(),
                                index1.intValue()
                        };
                        result.add(a);
